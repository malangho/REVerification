...
contract RoleManagementMin is Initializable {
    ...
    */
    ...
    IUserLogicMin public userLogicContract;
    ...
    modifier onlyRole(RoleManagementMin.Role _role) {
    ...
    modifier userExists(address _user){
        require(IRolesMin(address(userLogicContract)).doesUserExist(_user), "User does not exist");
        _;
    }
    ...
    modifier userHasRole(RoleManagementMin.Role _role, address _user){
    ...
    function initialize(address _userLogicContract) public initializer {
        userLogicContract = IUserLogicMin(_userLogicContract);
    }
    ...
    function isRole(RoleManagementMin.Role _role, address _caller) public view returns (bool) {
        /// @dev reading the rights for the user from the userDB-contract
        uint rights = IRolesMin(address(userLogicContract)).getRolesRights(_caller);
