...
import "./UserDefinitionPart.sol";

contract UserLogicPart is Initializable, RoleManagementPart, IRolesPart, IUserLogicPart {


    /// @notice mapping for addresses to users
    mapping(address => UserDefinitionPart.User) private userList;
    ...
    function initialize() public initializer {
        RoleManagementPart.initialize(address(this));

        // Set sender as User Admin
        UserDefinitionPart.User storage u = userList[msg.sender];
        u.roles = 2**uint(RoleManagementPart.Role.UserAdmin);
    }
    ...
    function deactivateUser(address _user)
        external
        onlyRole(RoleManagementPart.Role.UserAdmin)
    {
        require(
            !isRole(RoleManagementPart.Role.UserAdmin,_user)
            && !isRole(RoleManagementPart.Role.DeviceAdmin,_user),
            "user has an admin role at the moment"
        );

        UserDefinitionPart.User storage u = userList[_user];
        u.active = false;
    }
    ...
    {
        UserDefinitionPart.User memory user = userList[_user];
