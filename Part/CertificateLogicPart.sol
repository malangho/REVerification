...
contract CertificateLogicPart is Initializable, ERC721, ERC721Enumerable, RoleManagementPart, ICertificateLogic {

    bool private _initialized;
    IDeviceLogicPart private deviceLogic;
    ...
    mapping(address => bool) internal approvedExternalContracts;
    ...
    modifier onlyCertificateOwner(uint _certificateId) {
        require(
            ownerOf(_certificateId) == msg.sender || isRole(RoleManagementPart.Role.Matcher, msg.sender),
    ...
    function initialize(address _deviceLogicAddress) public initializer {
        ...
        deviceLogic = IDeviceLogicPart(_deviceLogicAddress);

        require(deviceLogic.userLogicAddress() != address(0), "initialize: deviceLogic hasn't been initialized yet.");


        RoleManagementPart.initialize(deviceLogic.userLogicAddress());
    ...
    function setApprovedExternalContract(address _contract) public{
        require(isRole(RoleManagementPart.Role.Issuer, msg.sender),"setApprovedExternalContract: caller is not an Issuer");
        approvedExternalContracts[_contract]=true;
    }
    ...
    */
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(
            _isApprovedOrOwner(msg.sender, tokenId) || isRole(RoleManagementPart.Role.Matcher, msg.sender),
    ...
    */
    ...
    function splitCertificate(uint certificateId, uint energy) public returns (uint childOneId, uint childTwoId) {
        require(
            msg.sender == ownerOf(certificateId) || isRole(RoleManagementPart.Role.Matcher, msg.sender),
    ...
    function requestCertificates(uint _deviceId, uint lastRequestedSMReadIndex) public {
        DeviceDefinitionsPart.Device memory device = deviceLogic.getDeviceById(_deviceId);

        require(device.owner == msg.sender, "msg.sender must be device owner");

        DeviceDefinitionsPart.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(_deviceId);
    ...
    function approveCertificationRequest(uint _certicationRequestIndex) public onlyRole(RoleManagementPart.Role.Issuer) {
        ...
        DeviceDefinitionsPart.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(request.deviceId);
        ...
        DeviceDefinitionsPart.Device memory device = deviceLogic.getDeviceById(request.deviceId);
    ...
    */
    ...
    function _claimCertificate(uint certificateId) internal {
        ...
        require(msg.sender == ownerOf(certificateId) || approvedExternalContracts[msg.sender], "_claimCertificate: You have to be the owner / an approved contract of the certificate.");
    ...
    function _createChildCertificates(uint parentId, uint energy) internal returns (uint childOneId, uint childTwoId) {
        ...
        DeviceDefinitionsPart.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(parent.deviceId);
