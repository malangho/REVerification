...
contract RoleManagementAll is Initializable {
    ...
    */
    ...
    IUserLogicAll public userLogicContract;
    ...
    modifier onlyRole(RoleManagementAll.Role _role) {
    ...
    modifier userExists(address _user){
        require(IRolesAll(address(userLogicContract)).doesUserExist(_user), "User does not exist");
        _;
    }
    ...
    modifier userHasRole(RoleManagementAll.Role _role, address _user){
    ...
    function initialize(address _userLogicContract) public initializer {
        userLogicContract = IUserLogicAll(_userLogicContract);
    }
    ...
    function isRole(RoleManagementAll.Role _role, address _caller) public view returns (bool) {
        /// @dev reading the rights for the user from the userDB-contract
        uint rights = IRolesAll(address(userLogicContract)).getRolesRights(_caller);
