...
contract CertificateLogicAll is Initializable, ERC721, ERC721Enumerable, RoleManagementAll, ICertificateLogic {

    bool private _initialized;
    IDeviceLogicAll private deviceLogic;
    ...
    modifier onlyCertificateOwner(uint _certificateId) {
        require(
            ownerOf(_certificateId) == msg.sender || isRole(RoleManagementAll.Role.Matcher, msg.sender),
    ...
    function initialize(address _deviceLogicAddress) public initializer {
        ...
        deviceLogic = IDeviceLogicAll(_deviceLogicAddress);

        require(deviceLogic.userLogicAddress() != address(0), "initialize: deviceLogic hasn't been initialized yet.");


        RoleManagementAll.initialize(deviceLogic.userLogicAddress());
    ...
    function setApprovedExternalContract(address _contract) public{
        require(isRole(RoleManagementAll.Role.Issuer, msg.sender),"setApprovedExternalContract: caller is not an Issuer");
    ...
    */
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(
            _isApprovedOrOwner(msg.sender, tokenId) || isRole(RoleManagementAll.Role.Matcher, msg.sender),
    ...
    */
    ...
    function splitCertificate(uint certificateId, uint energy) public returns (uint childOneId, uint childTwoId) {
        require(
            msg.sender == ownerOf(certificateId) || isRole(RoleManagementAll.Role.Matcher, msg.sender),
    ...
    function requestCertificates(uint _deviceId, uint lastRequestedSMReadIndex) public {
        DeviceDefinitionsAll.Device memory device = deviceLogic.getDeviceById(_deviceId);

        require(device.owner == msg.sender, "msg.sender must be device owner");

        DeviceDefinitionsAll.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(_deviceId);
    ...
    function approveCertificationRequest(uint _certicationRequestIndex) public onlyRole(RoleManagementAll.Role.Issuer) {
        ...
        DeviceDefinitionsAll.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(request.deviceId);
        ...
        DeviceDefinitionsAll.Device memory device = deviceLogic.getDeviceById(request.deviceId);
    ...
    */
    ...
    function _createChildCertificates(uint parentId, uint energy) internal returns (uint childOneId, uint childTwoId) {
        ...
        DeviceDefinitionsAll.SmartMeterRead[] memory reads = deviceLogic.getSmartMeterReadsForDevice(parent.deviceId);
